...
export const checkAvailability = async (
  ...
  switch (product.trackingType) {
    case ProductTrackingType.SERIALIZED:
      ...
      const existingSerials = await inventoryRepository.findSerialized(
        {
    ...
    case ProductTrackingType.LOT_TRACKED:
      ...
      const lotInventory = await inventoryRepository.findOneLot(
    ...
    case ProductTrackingType.EXPIRABLE:
      ...
      const expirableInventory = await inventoryRepository.findOneExpirable(
...
export const decreaseStock = async (params: StockAdjustmentParams) => {
  const {
    productId,
    warehouseId,
    quantity,
  ...
  const hasStock = await checkAvailability({ ...params, session });
  if (!hasStock) {
  ...
  await inventoryRepository.findOneAndUpdate(
    { productId, warehouseId },
  ...
  switch (product.trackingType) {
    case ProductTrackingType.SERIALIZED:
      await inventoryRepository.updateManySerialized(
        { serialNumber: { $in: serialNumbers! } },
        { isSold: true },
        { session }
      );
    ...
    case ProductTrackingType.LOT_TRACKED:
      await inventoryRepository.findOneAndUpdateLot(
    ...
    case ProductTrackingType.EXPIRABLE:
      await inventoryRepository.findOneAndUpdateExpirable(
...
export const increaseStock = async (params: StockAdjustmentParams) => {
  const {
    productId,
    warehouseId,
    quantity,
  ...
  switch (product.trackingType) {
    case ProductTrackingType.SERIALIZED:
      ...
      await inventoryRepository.insertManySerialized(serialEntries, {
        session,
      });
      break;
